...
Файл: `backend/app.js`  
Реализованы базовые маршруты регистрации/входа и **хеширование паролей**.
=======
### Занятие 7 — Регистрация и авторизация пользователей с хешированием паролей
Файлы: `backend/app.js`, `frontend/src/components/AuthForm.jsx`, `frontend/src/App.jsx`  
Добавлен полный цикл входа и регистрации пользователей.
>>>>>>> ca46f95654211b4a5829d0b32178a851af92197f

- Хеширование паролей реализовано через `bcrypt` (используется соль)
- Минимальные поля пользователя по методичке: `id`, `email`, `first_name`, `last_name`, `password` (хеш)
- Реализованы маршруты:
  - `POST /api/auth/register`
  - `POST /api/auth/login`
- Также реализованы CRUD-маршруты товаров (из таблицы задания):
  - `POST /api/products`
  - `GET /api/products`
  - `GET /api/products/:id`
  - `PUT /api/products/:id`
...
Реализована выдача JWT при входе и добавлен защищённый маршрут.

- При `POST /api/auth/login` сервер выдаёт `accessToken`
- Реализован защищённый маршрут:
  - `GET /api/auth/me` (возвращает текущего пользователя по `accessToken`)
- Защищены маршруты товаров (требуют `Authorization: Bearer <accessToken>`):
  - `GET /api/products/:id`
  - `PUT /api/products/:id`
  - `DELETE /api/products/:id`
...
Файлы: `backend/app.js`, `frontend/src/api/api.js`  
Реализовано обновление пары токенов через refresh-токен.

- При входе сервер выдаёт пару: `accessToken` + `refreshToken`
- Реализован маршрут:
  - `POST /api/auth/refresh`
- Refresh-токен берётся **из заголовка** `Authorization: Bearer <refreshToken>` (как указано в практическом задании)
...
Реализовано хранение токенов и автоматизация запросов через interceptors.
=======
- **user**:
  - просмотр каталога
  - работа с корзиной
  - без доступа к управлению товарами и пользователями
- **seller**:
  - все мозможности user
  - добавление/редактирование/удаление товаров
- **admin**:
  - все возможности seller
  - управление пользователями
- На сервере используются middleware авторизации и проверки роли для защищённых маршрутов
>>>>>>> ca46f95654211b4a5829d0b32178a851af92197f

- Токены хранятся в `localStorage` (`accessToken`, `refreshToken`)
- Перед каждым запросом токен подставляется в `Authorization`
- При `401` выполняется обновление токенов и запрос повторяется
- Фронтенд реализует вход/регистрацию и управление товарами через API
...
Реализованы роли и права доступа согласно таблице из методички.

- Middleware:
  - `authMiddleware` (проверка JWT)
  - `roleMiddleware` (проверка роли)
- Роли: `user` (пользователь), `seller` (продавец), `admin` (администратор)
- Минимальные маршруты и доступ:
